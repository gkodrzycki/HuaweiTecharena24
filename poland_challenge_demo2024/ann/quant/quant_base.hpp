...
concept QuantConcept =
    ...
    requires(Quant quant, const float *from, typename Quant::data_type *to) {
      { quant.encode(from, to) };
           ...
           {
             quant.get_computer(q)
             } -> std::same_as<typename Quant::ComputerType>;
         };

template <typename Quant>
concept SymComputableQuantConcept =
    QuantConcept<Quant> && requires(Quant quant) {
                             {
                               quant.get_sym_computer()
                               }
                               -> std::same_as<typename Quant::SymComputerType>;
                           };
