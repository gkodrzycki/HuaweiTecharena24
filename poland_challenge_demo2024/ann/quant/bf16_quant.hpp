...
#pragma omp parallel for schedule(dynamic)
  ...
  using ComputerType =
      SymComputer<typename Template::storage_type, metric, float, float, BF16>;

  auto get_computer(const float *query) const {
    return ComputerType(this->storage, query, *this);
  }

  constexpr static auto dist_func =
      metric == Metric::L2 ? helpa::l2_bf16_bf16 : helpa::dot_bf16_bf16;
  ...
  constexpr static auto dist_func =
      metric == Metric::L2
          ? helpa::l2_fp32_bf16
          : [](const float *x, const bf16 *y, const int32_t d) {
              return helpa::dot_fp32_bf16(x, y, d);
            };

  constexpr static auto dist_func_sym =
      metric == Metric::L2 ? helpa::l2_bf16_bf16
                           : [](const bf16 *x, const bf16 *y, const int32_t d) {
                               return helpa::dot_bf16_bf16(x, y, d);
                             };

  using ComputerType =
      ComputerImpl<Tensor, dist_func, float, float, float, bf16>;
