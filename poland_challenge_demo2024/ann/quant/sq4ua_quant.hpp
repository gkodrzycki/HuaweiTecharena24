...
template <Metric metric, typename CalibratorType = AffineCalibrator<15>,
          typename Template = Quantizer<metric, 128, 4>>
struct SQ4QuantizerUniformAsym : Template {
  ...
  void train(const float *data, int32_t n) {
    calibrator.calibrate(data, (int64_t)n * this->dim(), 0.01f);
  }
