...
#pragma omp parallel for schedule(dynamic)
  ...
  constexpr static auto dist_func =
      metric == Metric::L2 ? helpa::l2_fp32_fp16 : helpa::dota_fp32_fp16;

  constexpr static auto dist_func_sym =
      metric == Metric::L2 ? helpa::l2_fp16_fp16
                           : [](const fp16 *x, const fp16 *y, const int32_t d) {
                               return helpa::dot_fp16_fp16(x, y, d);
                             };

  using ComputerType =
      ComputerImpl<Tensor, dist_func, float, float, float, fp16>;
