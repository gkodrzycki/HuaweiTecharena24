...
ann_INLINE inline float reduce_add_f32x8(__m256 x) {
  auto sumh =
...
ann_INLINE inline int32_t reduce_add_i32x8(__m256i x) {
  auto sumh =
...
ann_INLINE inline int32_t reduce_add_i16x16(__m256i x) {
...
ann_INLINE inline __m256i load6(const uint16_t *code16) {
  const __m128i perm =
...
ann_INLINE inline __m256 decode_8_components(const uint8_t *code, int i) {
...
ann_INLINE inline __m256i cvti4x32_i8x32(__m128i x) {
