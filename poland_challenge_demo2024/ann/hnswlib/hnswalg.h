...
public:
  ...
  HierarchicalNSW(const QuantType &quant, size_t max_elements, size_t M = 16,
  ...
  std::priority_queue<pair_t> searchBaseLayer(tableint ep_id, int32_t u,
    ...
    std::priority_queue<pair_t> candidateSet;
    ...
    while (!candidateSet.empty()) {
      pair_t curr_el_pair = candidateSet.top();
      ...
      tableint x = curr_el_pair.second;
      ...
      for (int32_t j = 0; j < size; j++) {
        ...
        dist_t dist1 = computer(u, y);
        if (top_candidates.size() < ef_construction_ || lowerBound > dist1) {
          candidateSet.emplace(-dist1, y);
          top_candidates.emplace(dist1, y);
