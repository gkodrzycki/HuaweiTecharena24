...
template <DataType T> struct TypeTraits {};

template <> struct TypeTraits<DataType::fp32> {
  constexpr static int32_t nbits = 32;
};

template <> struct TypeTraits<DataType::fp16> {
...
template <> struct TypeTraits<DataType::e5m2> {
  constexpr static int32_t nbits = 8;
};

template <> struct TypeTraits<DataType::uint8> {
  constexpr static int32_t nbits = 8;
};

inline void convert_vector(DataType src_type, const char *src,
                           DataType dst_type, char *dst, int32_t d) {
